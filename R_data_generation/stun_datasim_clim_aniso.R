...
generate_stun_data <- function(
  ...
  script_time <- system.time(
    for (sim in 1:N_SIMS){
      ...
      # in the case of a constant stencil, this acts as a 2x2 diffusion matrix
      ...
      # once u get above 1 (like 1.001) you start to get basis func smoothing 
      # u can test this by printing values of f below
      #print(f[1:20,1])
...
n_replicates <- 30
N_SIMS <- 1000
DATA_LENGTH <- N_SIMS * (n_replicates + 3)
verbose <- FALSE 
sanity_plotting <- FALSE
...
# creating the initial dataset (only run the first time)
########################################################

file_name <- "data/STUN_data.h5"
# file_name <- "data/test_data.h5"
dataset_name <- "fields"
...
# writing to it for the first time
...
# Adding to the dataset (run this after the initial dataset is created)
######################################################################

# get the current number of sims (last number in dimensions)
file_name <- "data/STUN_data.h5"
dataset_name <- "fields"
print(h5ls(file_name))

# put it in this variable (could string parse this later)
# make sure to change this to the past size
# ex: if you just ran the script for the first time and made a 1000 fields
# when you run it again change this to 1000 so it adds to the h5 properly
nz_old <- 1000 
#rest of variables
nx <- rows*columns
ny <- n_replicates + 3
...
#extend the dataset
h5set_extent(file_name, dataset_name, c(nx, ny, nz))
...
write_time <- system.time(
  h5write(
    ...
    file = file_name, 
    name = dataset_name, 
    index = list(1:nx, 1:ny, (nz_old+1):nz)
  )
)
print(paste("Writing data to h5 took ", write_time[[3]]/60," minutes."))
#remember to close it 
